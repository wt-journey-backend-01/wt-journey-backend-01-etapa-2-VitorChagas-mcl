...
module.exports = {
  ...
  create(req, res) {
    ...
    const agenteCriado = agentesRepository.create({
      nome,
      dataDeIncorporacao,
      cargo,
    });
  ...
  update(req, res) {
    ...
    if ("cargo" in dadosAtualizados) {
      if (
        typeof dadosAtualizados.cargo !== "string" ||
        dadosAtualizados.cargo.trim() === ""
      ) {
        errors.push({
          field: "cargo",
          message: "Cargo deve ser uma string não vazia",
        });
      }
    }
    ...
    const agente = agentesRepository.update(id, {
      nome,
      dataDeIncorporacao,
      cargo,
    });
  ...
  partialUpdate(req, res) {
    ...
    if (
      "dataDeIncorporacao" in dadosAtualizados &&
      !isValidDate(dadosAtualizados.dataDeIncorporacao)
    ) {
      errors.push({
        field: "dataDeIncorporacao",
        message: "Data inválida ou no futuro",
